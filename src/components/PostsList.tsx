...
export default function PostsList() {
  ...
  const approvePost = async (id: string) => {
    ...
    // If LinkedIn is connected and the post platform is LinkedIn, post immediately
    const post = posts.find(p => p.id === id)
    if (post?.platform === 'linkedin' && linkedInConnected) {
      await postToLinkedIn(id)
      return
    }
  ...
  const postToLinkedIn = async (id: string) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        console.error('post-linkedin failed', response.status, text)
        const message = (result && result.error) || text || `Request failed with status ${response.status}`
